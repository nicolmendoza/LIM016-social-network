  ...
  Promise.all(postElements).then(() => {
    ...
    nuevoElemento.querySelectorAll('.edit').forEach((div) => {
      div.addEventListener('click', (e) => {
        ...
        for (let i = 0; i < post.length; i++) {
          if (post[i].userID === user.uid && post[i].idP === id) {
            console.log(document.querySelector(`#contentPost${id}`));
            ...
            document.querySelector('.save').addEventListener('click', () => {
              const postEdit = document.getElementById(`contentEdit${id}`).value;
              if (postEdit === `${post[i].content}`) {
                document.querySelector(`#contentPost${id}`).innerHTML = `${post[i].content}`;
              } else {
                console.log(document.getElementById(`contentEdit${id}`).value);
